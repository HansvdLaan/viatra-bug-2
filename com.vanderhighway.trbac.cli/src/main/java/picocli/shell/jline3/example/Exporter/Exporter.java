package picocli.shell.jline3.example.Exporter;
...
//import com.vanderhighway.trbac.patterns.SecurityZoneAccessible;
...
import picocli.shell.jline3.example.CLIContainer;
...
public class Exporter {
    ...
    public static void exportTopology(File outputFile, SiteAccessControlSystem system) throws IOException {
        ...
        for(SecurityZone zone: system.getSecurityZones()) {
    ...
    public static void exportTopologySmells(File outputFile, SiteAccessControlSystem system) throws IOException {
        Graph<SecurityZoneVertexTopology, DefaultEdge> exportGraph = new DefaultDirectedGraph<>(DefaultEdge.class);
        Map<String, SecurityZoneVertexTopology> vertexMap = new HashMap<>();
        ...
        try (FileOutputStream fos = new FileOutputStream(outputFile)) {
            for(Scenarios.Match scenarioMatch: modifier.getEngine().getMatcher(Scenarios.instance()).getAllMatches()) {
                if (i < max_scenario_count) {
                    ...
                    MyScenario myScenario = (MyScenario) modifier.getManipulation().createChild(schedule, modifier.getEPackage().getSchedule_MyScenarios(),
